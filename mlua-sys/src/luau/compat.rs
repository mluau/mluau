...
pub unsafe fn luaL_loadbufferenv(
    ...
    if mode.is_null() {
        lua_pushfstring(L, c"chunk mode must be explicitly set but a nullptr was passed to luaL_loadbufferenv mode; this is likely an mluau bug".as_ptr());
        return LUA_ERRRUN;
    }

    let chunk_is_text = {
        let modeb = CStr::from_ptr(mode).to_bytes();
        if modeb.contains(&b'b') {
            false
        } else if modeb.contains(&b't') {
            true
        } else {
            lua_pushfstring(L, c"mode doesn't contain t or b, we can't tell if it's bytecode or sourcecode; this is likely an mluau bug".as_ptr());
            return LUA_ERRRUN;
        }
    };
...
// deviaze:
// pub unsafe fn luaL_loadbuffer was removed because it passes nullptr to luaL_loadbufferenv
// which we don't allow anymore because chunk mode must be known by caller at load time
